  ...
  return (
    <>
      <header className='flex justify-between sticky top-0 p-4 items-center'>
        <h2 className='cursor-pointer font-bold text-2xl text-yellow-500 hover:text-yellow-600'>
          ...
          {authState.isLoggedIn ? (
            <>
              <li className="bg-blue-500 text-white hover:bg-blue-600 font-medium rounded-md">
                <Link to='/tasks/add' className='block w-full h-full px-4 py-2 bg-stone-100 hover:bg-stone-200 text-black'> <i className="fa-solid fa-plus"></i> Create task </Link>
              </li>
              <li className='py-2 px-3 cursor-pointer hover:bg-gray-200 transition rounded-sm' onClick={handleLogoutClick}>Logout</li>
            </>
          ) : (
            <div className='flex'>
              <li className='py-2 px-3 cursor-pointer text-yellow-500 hover:text-yellow-600 transition rounded-sm font-bold'><Link to="/login">Login</Link></li>
              <li className='py-2 px-3 cursor-pointer text-yellow-500 hover:text-yellow-600 transition rounded-sm font-bold'><Link to="/signup">SignUp</Link></li>
            </div>
          )}
        </ul>
